EXTERN JmpKiRetireDpcList:QWORD;		
EXTERN JmpRtlCaptureContext:QWORD;
...
EXTERN OnRtlCaptureContext:PROC;
 ...
 BackTo1942 PROC
                 ...
                 jmp     JmpRtlCaptureContext
BackTo1942 ENDP

 AdjustStackCallPointer PROC
    mov rsp, rcx
	xchg r8, rcx					;这里实际上不是为了交换r8和rcx的值，r8的值毫无意义，只是这里为了清零rcx，ExpWorkerThread的参数如果为0则会有判断处理
    jmp rdx
...
HookKiRetireDpcList PROC
                 ...
                 mov     [rax+168h], r9
                 mov     r11, [rsp]					
                 mov     [rax+40h], r11					;保存返回的Rip
                 mov     r11, rsp						
                 mov     [rax+48h], r11					;保存Rsp
				 lea     rax, RestoreCpuContext			
				 mov	 [rsp],rax						;把返回的Rip地址设置位RestoreCpuContext	
				 jmp     JmpKiRetireDpcList			
